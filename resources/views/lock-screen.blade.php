...
<body class="bg-silver-300">
    <div class="content">
        ...
        <div>
            ...
            <form class="text-center" id="lock-form" action="{{ route('unlock') }}" method="post">
                @csrf
                <h5 class="font-strong">{{ Auth::user()->name }}</h5>
                <p class="font-13">You are in lock screen. Enter your password to retrieve your session</p>
    ...
    <script>
        ...
        // Increment the inactivity time counter every second
        const timerIncrement = setInterval(() => {
            inactivityTime++;
            if (inactivityTime >= 300) { // 300 seconds = 5 minutes
                window.location.href = "{{ route('lock-screen') }}"; // Redirect to lock screen
            }
